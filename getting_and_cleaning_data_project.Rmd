...
head(tidy_full_data_set)
names <- colnames(tidy_full_data_set)
unique(names)#Seems to have duplicats
select_unique<-!duplicated(names)
select_unique

tidy_treated_data_set <- tidy_full_data_set[,select_unique==TRUE]
dim(tidy_treated_data_set)
head(tidy_treated_data_set)
...
as.character(activity_labels[1, 2])

tidy_treated_data_set <- tidy_treated_data_set %>%
  rename(y_activity = y) %>% 
